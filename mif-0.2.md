# MIF Community Working Draft 0.2
...
b) MFEN/0.2, a canonical textual representation of an instantaneous state;

c) MPK/0.2, a canonical textual key for structural analysis under an explicit
key profile;

d) MSTATE/0.2, an I-JSON representation of an origin, an event sequence, a
current checkpoint, repetition observations and claims;

e) MRS/0.2, a canonical ruleset manifest that selects the finite rule
...
# 7 MFEN/0.2 — Instantaneous position

## 7.1 Purpose

MFEN/0.2 shall represent the exact instantaneous rules state defined by
...
- use the exact signature `MFEN/0.2`;
...
```text
MFEN/0.2 mill24-state-v1 x-mif-fixture-nmm@1 ......../......../........ w p p 9,9 - 0 0 - rh=sha256:2816857af70daf493797292a795bafb933a7294a0c7d97a8f99fde63974bf8cd
```
...
```text
MFEN/0.2 mill24-state-v1 x-mif-fixture-nmm@1 WWW...B./......B./........ w p r 6,7 w:mill:b:b:1:004040:b 0 5 - rh=sha256:2816857af70daf493797292a795bafb933a7294a0c7d97a8f99fde63974bf8cd
```
...
```text
MFEN/0.2 mill24-state-v1 x-mif-fixture-dooz@1 WWW...../B......B/........ w p r 9,10 w:mill:h:b:1:-:b 0 5 - rh=sha256:f37eb3bf82877d747000ea08951470f70cc61f4a861830c396ee62a0399f503d
```
...
```text
MFEN/0.2 mill24-state-v1 x-mif-fixture-delay@1 WWW...../b......B/........ b p p 9,10 - 0 5 - rh=sha256:3da403256d96a2da0a6a8cc579d3cc1555fa4a802dea1672fadf89a481b3d3d9
```

### 7.4.5 Choice branches and sequence-dependent fields

```text
MFEN/0.2 mill24-state-v1 x-mif-fixture-stateful@1 BWBW.WB./.W.W..B./.W...... w p r 3,5 w:intervention:b:b:2:000005:b|w:custodian:b:b:1:000040:b|w:mill:b:b:1:004000:b 7 20 - lm=-,-;-,- rh=sha256:ce2bf7b8928a953c839d595ed141fb02523d71424ba4c256f902af2364713075 ul=0000,0000
```
...
# 8 MPK/0.2 — Stable structural analysis key

## 8.1 Purpose and limits

MPK/0.2 provides a stable textual key for structural analysis under an
...
```text
MPK/0.2 <state-profile> <ruleset> <key-profile> <board24>
...
```text
MPK/0.2 mill24-state-v1 x-mif-fixture-nmm@1 structural-d4-v1 .......W................ b p 8,9 rh=sha256:2816857af70daf493797292a795bafb933a7294a0c7d97a8f99fde63974bf8cd
```
...
```text
MPK/0.2 mill24-state-v1 x-mif-fixture-nmm@1 structural-aut16-v1 .......................W b p 8,9 rh=sha256:2816857af70daf493797292a795bafb933a7294a0c7d97a8f99fde63974bf8cd
```

These are intentionally different keys. A database shall identify which key
profile it uses.

# 9 MSTATE/0.2 — Resumable game state

## 9.1 Purpose

MSTATE/0.2 represents:
...
| Member | Type | Meaning |
|---|---|---|
| `format` | string | `MSTATE/0.2` |
| `positionFormat` | string | format of `origin` and `current`; `MFEN/0.2` in this edition |
...
A gameplay event by `w` or `b` shall match the current action, side and
obligation. `system` shall not perform `place`, `move`, `remove`, draw
negotiation, claim or resignation events.
...
|---|---|---|---|
| `offer-draw` | active player | none | open an offer |
| `accept-draw` | other player | `offerEventSeq` | terminal draw by agreement |
| `decline-draw` | other player | `offerEventSeq` | close offer as declined |
| `withdraw-draw` | offerer | `offerEventSeq` | close offer as withdrawn |
| `claim-draw` | active player | `reason` (`no-progress` or `repetition`) | accept only if the selected claim condition is currently satisfied |
...
The array shall be in strictly increasing `eventSeq` order. `eventSeq` is
unique in the array. Multiple records of the same kind are allowed when they
...
3. parse and validate origin;
4. take only leading `pre-origin` repetition entries as initial history;
5. initialize an empty claim audit;
6. generate the origin observation if applicable;
...
# 10 MRS/0.2 — Finite ruleset manifest

## 10.1 Design boundary

MRS/0.2 follows the finite-mechanism approach.
...
An MRS/0.2 manifest has no order-sensitive mechanism array. Trigger and
...
`movingEffect` shall be `remove-opponent-board` in MRS/0.2.
...
No MRS/0.2 mechanism uses a point-union substitute for `used-lines`.
...
A movement that forms no usable line clears that player's `lm` to `-,-`.
...
Let `R` be the mill removal multiplicity.

### 11.9.1 `remove-opponent-board`

Create one board obligation for the forming player against the opponent with
remaining `R` and targets from 11.6. The opponent is the next primary player.
...
2. if `R` exceeds the opponent hand, append a board obligation for the
   remainder, using targets `~` while it follows the hand obligation;
...
remaining is `R`, and next primary player is the opponent.
...
player against the opponent. Remaining is the declared multiplicity. The
opponent is the next primary player after the branch.
...
- `change-player`: side changes to the opponent;
...
- `checkpoint-mismatch`;
- `repetition-history-mismatch`; and
- `claims-mismatch`.
...
actions, replay, outcome or a key.
...
All quoted literals are case-sensitive.
...
The standalone `conformance/mif-0.2.abnf` file is the machine-readable copy.
...
### F.3 Why phase and action remain
...
### F.5 Why obligations remain in MFEN
...
### F.6 Why two structural key profiles exist
...
### F.7 Conditions before a stable candidate
